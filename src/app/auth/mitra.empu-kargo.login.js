...
const TITLE = 'LOGIN - EMPU KARGO';
const INTERNAL_DATA_NAME = process.env.EXPO_PUBLIC_ASYNC_MEMBER_ACCOUNT_EMPU_CARGO;
const BACKEND_LOGIN_URL = `${process.env.EXPO_PUBLIC_BACKEND_URL}/mitra-empu-kargo/login`;
const REDIRECT_SUCCESS = '/mitra-content/empu-kargo';
...
export default () => {
  ...
  const loginDev = async () => {
    try {
      ...
      const device_id = await SecureStore.getItemAsync('deviceId');
      const { data } = await axios({
        url: `${process.env.EXPO_PUBLIC_BACKEND_URL}/mitra-empu-kargo/${process.env.EXPO_PUBLIC_ASYNC_STORAGE_DEV_LOGIN_PATH}`,
        method: 'POST',
        data: {
          device_id: device_id
        }
