...
const TITLE = 'LOGIN - OPERATOR AIRLINES';
const INTERNAL_DATA_NAME = process.env.EXPO_PUBLIC_ASYNC_MITRA_ACCOUNT_OPERATOR_AIRLINES;
const BACKEND_LOGIN_URL = `${process.env.EXPO_PUBLIC_BACKEND_URL}/mitra-operator-airlines/login`;
const REDIRECT_SUCCESS = '/mitra-content/operator-airlines';
...
export default () => {
  ...
  const loginDev = async () => {
    try {
      ...
      const device_id = await SecureStore.getItemAsync('deviceId');
      const { data } = await axios({
        url: `${process.env.EXPO_PUBLIC_BACKEND_URL}/mitra-operator-airlines/${process.env.EXPO_PUBLIC_ASYNC_STORAGE_DEV_LOGIN_PATH}`,
        method: 'POST',
        data: {
          device_id: device_id
        }
