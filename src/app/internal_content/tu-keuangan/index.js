...
import React, { useEffect, useContext, createContext, useState, Fragment, useFocusEffect } from 'react';
...
export default () => {
  ...
  const homeBackUrl  = '/(tabs)/internal';
  const { title, user, basepath, baseApiUrl, logout } = useLayoutContext();
  const url = 'https://editor.deoairport.co.id/mobile/apps/android_tu_keuangan/home';
...
export default () => {
  ...
  const menu = [

    {
      ...
      listMenu: [
        {
          title: 'DATA ON PROGGRESS',
          href: '/data-amc-on-proggress',
          icon: { component: FontAwesome, name: 'fire-extinguisher' }
        },
        {
          title: 'MENUNGGU VERIFIKASI',
          href: '/data-amc-menunggu-verifikasi',
...

    {
      ...
      listMenu: [
        {
          title: 'DATA - ALL FINAL',
          href: '/all-amc-final',
          icon: { component: FontAwesome, name: 'fire-extinguisher' }
        },
        {
          title: 'DATA - ALL CANCEL',
          href: '/all-amc-cancel',
          icon: { component: FontAwesome, name: 'fire-extinguisher' }
        },
        {
  ...
  return (<SafeAreaLayout style={{ flex: 1 }} insets='top'>
    ...
    <View className=" flex-row items-center justify-between p-2">
      <TouchableOpacity className="p-1" onPress={() => router.push('/internal')}>
