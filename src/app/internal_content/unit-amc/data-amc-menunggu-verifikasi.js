...
export default () => {
  ...
  if(viewMode == 'teruskan-ke-operator'){
    return (<TeruskanKeOperator 
        title={title} 
        user={user} 
  ...
  if(viewMode == 'perbaiki-data'){
    return (<KembalikanData 
        title={title} 
        user={user} 
        baseApiUrl={baseApiUrl} 
        setViewMode={setViewModeCallback} 
...
const TeruskanKeOperator = memo(({ setViewMode, title, baseApiUrl, user, reloadData = () => { }, itemFocus, setItemFocus }) => {
 
  
  const populateForm = {
    is_verified_kanit : true
  };

  const custom_fields = {
    catatan_final: {
      label: 'CATATAN UNTUK OPERATOR',
      type: "text"
    },
  };
...
const KembalikanData = memo(({ setViewMode, title, baseApiUrl, user, reloadData = () => { }, itemFocus, setItemFocus }) => {
  ...
  const menu = [
    {
      ...
      onPress : () =>{
        setItemFocus(item);
        setViewMode('teruskan-ke-operator')
      }
    },
    {
      title : 'PERBAIKI DATA',
